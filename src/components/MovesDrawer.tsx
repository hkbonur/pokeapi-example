...
function MoveItem({ move }: MoveItemProps) {
  ...
  return (
    <div className="border-b border-gray-200 last:border-b-0">
      ...
      >
        <span className="font-medium text-gray-800 capitalize text-left">{moveName.replace(/-/g, ' ')}</span>
        <div className="flex items-center gap-2">
          {isExpanded && isLoading && <Loader2 size={16} className="animate-spin text-gray-400" />}
          {isExpanded ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-gray-800">{moveData.power ?? '—'}</p>
                </div>
                <div className="bg-white rounded-lg p-3">
                  <p className="text-xs text-gray-500 font-semibold uppercase">Accuracy</p>
                  <p className="text-lg font-bold text-gray-800">{moveData.accuracy ? `${moveData.accuracy}%` : '—'}</p>
                </div>
                <div className="bg-white rounded-lg p-3">
                  <p className="text-xs text-gray-500 font-semibold uppercase">PP</p>
                  <p className="text-lg font-bold text-gray-800">{moveData.pp ?? '—'}</p>
                </div>
                <div className="bg-white rounded-lg p-3">
                  <p className="text-xs text-gray-500 font-semibold uppercase">Priority</p>
                  <p className="text-lg font-bold text-gray-800">{moveData.priority ?? 0}</p>
                </div>
              </div>
              ...
              {moveData.effect_entries && moveData.effect_entries.length > 0 && (
                <div className="bg-white rounded-lg p-3">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed">
                    {moveData.effect_entries[0].short_effect || moveData.effect_entries[0].effect}
                  </p>
                </div>
              )}

              {/* Flavor Text */}
              {moveData.flavor_text_entries && moveData.flavor_text_entries.length > 0 && (
                <div className="bg-white rounded-lg p-3">
...
export function MovesDrawer({ isOpen, onClose, moves, pokemonName }: MovesDrawerProps) {
  // Close on Escape key
  useEffect(() => {
      ...
      >
        ...
        <div className="bg-linear-to-r from-red-500 to-red-600 p-6 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-white/90 text-sm mt-1">{moves.length} moves available</p>
          </div>
          <button
            onClick={onClose}
