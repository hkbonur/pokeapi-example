      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden max-w-md mx-auto transform transition-all hover:scale-105">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="text-center">
            <h2 className="text-4xl font-black text-gray-800 capitalize">{pokemon.name}</h2>
            <p className="text-gray-500 font-bold">#{String(pokemon.id).padStart(3, '0')}</p>
          </div>
            ...
            {pokemon.stats?.slice(0, 6).map((stat) => (
              <div key={stat.stat.name} className="space-y-1">
                <div className="flex justify-between text-sm">
                  <span className="font-semibold text-gray-600 capitalize">{stat.stat.name.replace('-', ' ')}</span>
